...
<img src="https://tva1.sinaimg.cn/large/006y8mN6gy1g8aiqpaupkj30y60gu0xd.jpg" alt="image-20191025161921636" style="zoom:50%;" />
  ...
  7. # 数据规划
     ...
     | 数据                     | 源                         | 获取方式 | 目标                    | 备注                     |
     | ------------------------ | -------------------------- | -------- | ----------------------- | ------------------------ |
     | 历史交易相关数据         | 支付、充值、计费中心后线库 | 离线     | 数据中台                |                          |
     | 当天交易相关数据         | 支付、充值、计费中心接口   | 实时     | 各业务前台              | 只适合简单查询           |
     |                          | 报文日志                   | 近实时   | 数据中台→各业务前台     |                          |
     |                          | 支付、充值、计费中心前线库 | 离线     | 对账后线库              |                          |
     | 历史对账、结算、稽核信息 | 对账、结算中心数据库       | 离线     | 数据中台→各业务前台     |                          |
     | 当天对账、结算、稽核信息 | 对账、结算中心接口         | 实时     | 各业务前台              | 只适合简单查询及简单统计 |
     | 告警信息                 | 所有服务的告警日志         | 实时     | 监控中心告警系统        |                          |
     | 应用状态(内存、线程等)   | 各服务接口                 | 实时     | 运维前台、可视化等      |                          |
     | 调用链状态               | 各服务接口                 | 实时     | 服务治理的服务→运维前台 |                          |
     | …                        |                            |          |                         |                          |
  ...
  8. # 进度规划
     ...
     2020年规划如下，依附于一级充值支付中心二期及新业务开发工作：

     | 任务层                           |      | 任务                                                         |
     | -------------------------------- | ---- | ------------------------------------------------------------ |
     |                                  |      |                                                              |
     | 基础设施                         |      | 现有支付、计费系统迁移云平台     新建和改造的所有服务均以容器方式部署在云平台 |
     |                                  |      | 统一应用框架                                                 |
     | 后台                             |      | 统一公参                                                     |
     |                                  |      | 虚拟账户                                                     |
     |                                  |      | 加解密                                                       |
     ...
     | 对外服务（商户服务部分）         |      |                                                              |
     | 充值赋能（无需大变，按计划演进） |      |                                                              |
     | 服务治理                         |      | 所有功能；对已完成的服务统一治理                             |
     | 运维保障                         |      | 增加服务治理的链路跟踪结果展示                               |
