...
![image-20191205103948537](https://tva1.sinaimg.cn/large/006tNbRwgy1g9lnc4co4jj316m0gudoj.jpg)
...
- 技术中台：将原个业务系统相似的、业务无关的功能统一成独立服务，供各中台复用。
  ...
  - 数据同步：集中管理充值、支付以及计费业务各个环境前线库和后线库之间的离线数据同步服务，通过统一的管理前台增加、修改定时数据同步任务，包括数据库到数据库、数据库到文件、文件到数据库。并且可以实时查看数据库同步的状态是成功、备份中还是失败。具备失败重跑、异常告警功能。具体的同步任务由各个worker节点执行，具有高可用的特点。
